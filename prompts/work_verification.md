...
This document defines the official verification, audit, and quality assurance protocol for all agents and developers. It ensures systemic integrity, eliminates redundancies, and enforces strict execution standards by heavily cross-referencing the rules defined in [`tasks.md`](file:///c:/Akshat/ContAIned/agent_buildable_base/tasks/tasks.md) and [`references.md`](file:///c:/Akshat/ContAIned/agent_buildable_base/references/references.md).
...
* **Evaluation & Guardrails:** Ensure that benchmarking tests (`poetry run deepeval run test <file>`) and safety checks pass successfully for any new AI feature or agent logic.
...
* **Data Schemas & Type Safety:** Validate that all backend models (e.g., Pydantic v2, SQLAlchemy v2) are strictly typed (`poetry run mypy .`) and accurately reflect the architectural intent.
...
* **Containerization & Deployment:** Ensure that Dockerfiles, container orchestrations, and Poetry dependency definitions in [`pyproject.toml`](file:///c:/Akshat/ContAIned/pyproject.toml) build the updated environment cleanly (`poetry check`, `poetry install --all-extras`).
...
1. **Identify the Target Perimeter:** Retrieve active file changes or git commit diff associated with the completed Subtask.
...
graph TD
    ...
    F -- No Gaps --> H[Run Poetry Verification Tests]
...
### Step 1: Trace Execution to Specification (Adhere to [`tasks.md`](file:///c:/Akshat/ContAIned/agent_buildable_base/tasks/tasks.md))
* Map active code changes directly to the "Definition of Done" outlined in the specific Subtask file.
...
* **Evolve the Baseline:** If an architectural oversight or anti-pattern is repeatedly discovered, update [`prompts/work_principle.md`](file:///c:/Akshat/ContAIned/agent_buildable_base/prompts/work_principle.md) to evolve system baseline intelligence.
...
| **Static Code Inspection** | File Content Review | Review logic blocks for proper type definitions, Pydantic validations, telemetry integration, and Poetry usage. |
...
| **Verify Test Quality** | Environment Test Execution | Run Poetry test commands (`poetry run pytest`, `poetry run deepeval`) to guarantee AI reliability and backend system health. |