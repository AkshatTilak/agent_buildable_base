...
* `[x] sub/state_new_zustand_slices.md` — Create settingsSlice, notificationSlice, and ingestionSlice.
* `[x] sub/state_api_client_hardening.md` — Replace all `any` in api.ts with typed interfaces, read settings from store, add retry logic.
* `[x] sub/state_telemetry_settings_sync.md` — Telemetry service reads gateway URL from settings store; add history buffer for sparklines.
