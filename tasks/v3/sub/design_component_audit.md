...
## Actionable Steps
- [ ] Audit every component file in `frontend/src/components/` for hardcoded hex values in Tailwind classes.
- [ ] Replace all inline hardcoded colors with design token equivalents:
...
- [ ] Ensure all buttons follow the button tier system (primary/secondary/ghost/danger).
- [ ] Ensure all form inputs follow the input pattern from design system.
- [ ] Ensure all modals follow the modal pattern (blur overlay, proper header/footer).
- [ ] Ensure font usage: headings use `--font-display`, labels are uppercase with tracking.
