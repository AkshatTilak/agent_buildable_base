...
## Actionable Steps
- [x] `projects/syntraflow/api.py` — Add `GET /documents` endpoint:
...
- [x] `projects/syntraflow/api.py` — Add `GET /jobs` endpoint:
...
- [x] Add `GET /documents/{doc_id}/chunks` endpoint for viewing chunk details of a specific document.
- [x] Update `frontend/src/services/api.ts` — Add `getDocuments()`, `getJobs()`, `getDocumentChunks()` methods.
