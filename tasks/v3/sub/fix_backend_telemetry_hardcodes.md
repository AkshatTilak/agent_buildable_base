...
## Actionable Steps
- [x] `gateway/api/telemetry.py:41` — Replace hardcoded `vram_usage_mb: 4096` with actual GPU memory query:
...
- [x] `gateway/api/telemetry.py:42` — Replace hardcoded `vram_total_mb: 16384` with actual total.
- [x] `gateway/api/telemetry.py:43` — Replace hardcoded `active_agents: 2` with actual count from:
  - Query `AgentDefinition` table count, OR
  - Read from `AgentRuntimeManager` cache size.
- [x] Add `disk_usage_percent` metric to telemetry response using `psutil.disk_usage('/')`.
- [x] Add `active_jobs_count` to telemetry by querying `SyntraFlowJob` table for status='processing'.
- [x] Add `gpu_available` boolean to telemetry response.
