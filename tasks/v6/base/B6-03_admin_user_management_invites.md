...
- [x] `user_invites` exists with hashed single-use tokens, TTL, `platform_role`, `hub_grants_json`, and a partial unique index allowing one open invite per email.
- [x] Invite redemption works via **both** OAuth ("Continue with Google") and password setup; a provider email mismatch is rejected with `409`.
- [x] Redeeming an invite applies its `hub_grants_json` as real `hub_members` rows and activates the account immediately.
