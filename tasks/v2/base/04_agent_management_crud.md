...
## Subtask Registry
* `[ ] sub/agent_crud_schema.md` — Define Pydantic models and SQLAlchemy schemas for `AgentDefinition` (Prompts, Model ID, Tools, Hyperparams).
* `[ ] sub/agent_crud_api.md` — Build standard REST APIs in the Gateway for listing, creating, and updating Agent configurations.
* `[ ] sub/agent_hub_ui.md` — Build the Frontend "Agent Hub" grid and detail view for managing agents.
* `[ ] sub/agent_runtime_sync.md` — Ensure that when an Agent is updated in the DB, the running Inference Server and GuardRoute orchestrator reload its configuration.
