...
1. **Test Infrastructure & Configuration Foundation**:
   - `.env.test` (gitignored) + `.env.test.example` (committed) targeting actual running dev infrastructure & ports (`:8000` Gateway, `:8001` Inference, standard DB ports).
   ...
   - Subprocess/client fixtures for Gateway (`:8000`) and Inference (`:8001`) without separate ports or duplicate container stacks.
...
- Real-service integration, streaming, E2E, live API, and performance tests passing against actual running dev services and ports (`:8000`, `:8001`, standard DB ports).
