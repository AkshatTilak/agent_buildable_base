...
## Tasks
1. [x] `ws_connect` fixture — async context manager for WebSocket connections via `websockets` library.
2. [x] `sse_connect` fixture — async HTTP client that reads `text/event-stream` responses and yields parsed events.
3. [x] `wait_for_event` helper — waits for a specific SSE event type with timeout.
4. [x] `collect_all_events` helper — collects all SSE events until stream closes or timeout.
