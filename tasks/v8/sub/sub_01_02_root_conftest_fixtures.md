...
3. [x] Provide `real_db_session` async fixture:
   - Connects to a real Postgres test database.
   - Wraps each test in a transaction and rolls back.
4. [x] Provide `qdrant_client`, `redis_client`, `neo4j_driver` fixtures connected to real services.
5. [x] Provide `gateway_client` fixture (httpx `AsyncClient` against a running or TestClient-mounted gateway).
6. [x] Provide `seed_user`, `seed_hub`, `seed_workflow` factory fixture chain for ordered data creation.
