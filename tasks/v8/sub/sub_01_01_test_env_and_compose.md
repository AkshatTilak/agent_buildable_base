...
## Tasks
1. [ ] Create `.env.test` (gitignored) with real service credentials:
   ...
   - `AUTO_APPROVE_EMAIL_DOMAINS` (empty to exercise approval gate).
   - `OCR_PROVIDER` (local/off).
2. [ ] Create `.env.test.example` (committed) with placeholder values and documentation of each variable.
3. [ ] Create `infrastructure/docker-compose.test.yml`:
