...
## Tasks
1. [x] Run `pytest tests/unit/ -v` and confirm it reproduces the same pass/fail as today.
2. [x] Tag every test file with the appropriate marker (`@pytest.mark.unit`, `@pytest.mark.integration`, etc.).
3. [x] Fix any tests that break during migration (imports, fixtures, config).
4. [x] Confirm `--strict-markers` passes with all registered markers.
