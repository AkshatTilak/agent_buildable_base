...
## Tasks
1. [x] Use React Query's built-in deduplication (`useQuery` with `staleTime`).
2. [x] Add `staleTime` and `gcTime` to all query hooks.
3. [x] Implement optimistic updates for hub/agent/workflow CRUD.
4. [x] Batch related API calls on page load (e.g., hub metadata + members + links in one request, or parallel `Promise.all`).
